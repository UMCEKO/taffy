...
fn compute_inner(
    ...
    // An aspect-ratio with a definite width makes the automatic height
    // definite (css-sizing-4): children resolve percentage heights against
    // it, and the container does not grow to its content height.
    // Final-layout only: widths in measure passes are tentative and must
    // not transfer through the ratio (intrinsic contributions stay
    // content-based).
    let aspect_derived_height = match (run_mode, size.height, aspect_ratio) {
        (RunMode::PerformLayout, None, Some(ratio)) => {
            Some((container_outer_width / ratio).maybe_clamp(min_size.height, max_size.height))
        }
        _ => None,
    };
    ...
    let container_percentage_resolution_height = known_dimensions
        ...
        .or(aspect_derived_height)
        .or(min_size.height);

    // Item sizes were resolved against the container's known dimensions,
    // whose height was unknown when it is aspect-derived. Re-resolve the
    // height components so percentage heights become definite.
    if aspect_derived_height.is_some() && container_content_box_size.height.is_none() {
        let inner_size = Size {
            width: Some(container_outer_width - content_box_inset.horizontal_axis_sum()),
            height: container_percentage_resolution_height
                .map(|height| height - content_box_inset.vertical_axis_sum()),
        };
        for item in items.iter_mut() {
            let child_style = tree.get_block_child_style(item.node_id);
            let child_aspect_ratio = child_style.aspect_ratio();
            let pb_sum = (item.padding + item.border).sum_axes();
            let box_sizing_adjustment =
                if child_style.box_sizing() == BoxSizing::ContentBox { pb_sum } else { Size::ZERO };
            item.size = child_style
                .size()
                .maybe_resolve(inner_size, |val, basis| tree.calc(val, basis))
                .maybe_apply_aspect_ratio(child_aspect_ratio)
                .maybe_add(box_sizing_adjustment);
            item.min_size = child_style
                .min_size()
                .maybe_resolve(inner_size, |val, basis| tree.calc(val, basis))
                .maybe_apply_aspect_ratio(child_aspect_ratio)
                .maybe_add(box_sizing_adjustment);
            item.max_size = child_style
                .max_size()
                .maybe_resolve(inner_size, |val, basis| tree.calc(val, basis))
                .maybe_apply_aspect_ratio(child_aspect_ratio)
                .maybe_add(box_sizing_adjustment);
